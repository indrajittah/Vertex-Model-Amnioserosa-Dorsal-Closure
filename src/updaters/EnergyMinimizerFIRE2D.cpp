...
void EnergyMinimizerFIRE::fireStepCPU()
    ...
    if (true)//scope for array handles
        ...
        for (int i = 0; i < N; ++i)
            {
	    //cout<<h_f.data[i].x<<h_f.data[i].y<<endl;
...
void EnergyMinimizerFIRE::minimize()
	...
	//A_Periodic = 1.0+0.3*sin(100.0*0.001*iterations);
	//cout<<iterations<<"\t"<<sqrt(forceMax)<<endl;
	//ArrayHandle<Dscalar2> h_m(Moduli,access_location::host,access_mode::overwrite);
	//State->setCellPreferencesUniform(A_Periodic,3.70);
	//State->setModuliUniform(1.0,A_Periodic);
        iterations +=1;
        velocityVerlet();
        fireStep();
	//printf("step %i max force:%.3g \tpower: %.3g\t alpha %.3g\t dt %g\t %.14f \t avgA %.6f\t avgP %.6f\n",iterations,sqrt(forceMax),Power,alpha,deltaT,State->computeEnergy(),State->reportavgA(),State->reportavgP());
        //printf("%i\t%.12f\n",iterations,State->reportavgA());
