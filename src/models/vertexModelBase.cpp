...
void vertexModelBase::OOPCPU_Individual()
    {
    ArrayHandle<Dscalar2> h_p(cellPositions,access_location::host,access_mode::readwrite);
    ArrayHandle<Dscalar2> h_v(vertexPositions,access_location::host,access_mode::read);
    ArrayHandle<int> h_nn(cellVertexNum,access_location::host,access_mode::read);
	...
	Dscalar Area = 0.0;
        Dscalar2 centroid = zero;
        Dscalar2 MajMinaxissquare = zero;
        Dscalar sxx=0.0;
        Dscalar syy=0.0;
        Dscalar sxy=0.0;
	for (int vv = 0; vv < neighs; ++vv){
		Area += (vertices[vv].x*vertices[vv+1].y - vertices[vv+1].x*vertices[vv].y);
            centroid.x += (vertices[vv].x+vertices[vv+1].x) * (vertices[vv].x*vertices[vv+1].y - vertices[vv+1].x*vertices[vv].y);
            centroid.y += (vertices[vv].y+vertices[vv+1].y) * (vertices[vv].x*vertices[vv+1].y - vertices[vv+1].x*vertices[vv].y);
            sxx += ((vertices[vv].y*vertices[vv].y) + (vertices[vv].y*vertices[vv+1].y) + (vertices[vv+1].y*vertices[vv+1].y)) * ((vertices[vv].x*vertices[vv+1].y) - (vertices[vv+1].x*vertices[vv].y ));
            syy += ((vertices[vv].x*vertices[vv].x) + (vertices[vv].x*vertices[vv+1].x) + (vertices[vv+1].x*vertices[vv+1].x)) * ((vertices[vv].x*vertices[vv+1].y) - (vertices[vv+1].x*vertices[vv].y ));
            sxy += (vertices[vv].x*vertices[vv+1].y + 2.0*vertices[vv].x*vertices[vv].y + 2.0*vertices[vv+1].x*vertices[vv+1].y + vertices[vv+1].x*vertices[vv].y)*(vertices[vv].x*vertices[vv+1].y - vertices[vv+1].x*vertices[vv].y);
         };
	
	Area = 0.5*Area;
	centroid.x = centroid.x / (6.0*Area);
        centroid.y = centroid.y / (6.0*Area);
        sxx = sxx/12 - (Area*centroid.y*centroid.y);
        syy = syy/12 - (Area*centroid.x*centroid.x);
        sxy = sxy/24 - (Area*centroid.x*centroid.y);
	Eigen::Matrix<double, 2, 2> X;
//	This is symmetric matrix, as X_ij = X_ji
//	Self adjoint matrix
        X(0,0) = sxx; X(0,1) = sxy; X(1,0) = sxy; X(1,1) = syy;
        Eigen::MatrixXd A = X + X.transpose();
        Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> s(A);
	Eigen::VectorXd SEV = s.eigenvectors().col(0); //Eigen vector corresponding to major axis direction
        SEV.normalize();//normalized eigen vector	
        Eigen::VectorXd RefVec(2,1); //RefVec(Row,Column)
        RefVec(0,0) = 1; RefVec(1,0) = 0; //correspond to x-axis or Anterior-Posterioir axis
//      To obtain the orientation of the cells, we simply calculate the angle of the major axis towards the x-axis:
	Dscalar costheta = SEV.dot(RefVec);
        //OOP += (3.*costheta*costheta-1)/2.; //In 3d
	//OOP += (2.*costheta*costheta-1); //2d 
	
	h_p.data[cell].x = (2.*costheta*costheta-1);
	h_p.data[cell].y = 0.0;
	};
	//cout<<"OOP = "<<OOP/(Dscalar)Ncells<<endl;
	//return OOP/(Dscalar)Ncells;
    };


Dscalar vertexModelBase::OOPCPU()
    {
    ArrayHandle<Dscalar2> h_p(cellPositions,access_location::host,access_mode::readwrite);
    ArrayHandle<Dscalar2> h_v(vertexPositions,access_location::host,access_mode::read);
    ArrayHandle<int> h_nn(cellVertexNum,access_location::host,access_mode::read);
...
//      To obtain the orientation of the cells, we simply calculate the angle of the major axis towards the x-axis:
	Dscalar costheta = SEV.dot(RefVec);
        //OOP += (3.*costheta*costheta-1)/2.; //In 3d
	OOP += (2.*costheta*costheta-1); 

//        Dscalar theta = acos(costheta)*(180.0 / 3.14159265);
//	OOP = cos(2.0*theta*(3.14159265/180.0));
//	OOP += cos(2.0*acos(costheta)); //In 2d
	};
	//cout<<"OOP = "<<OOP/(Dscalar)Ncells<<endl;
	return OOP/(Dscalar)Ncells;
    };
...
void vertexModelBase::getCellmajorandminoraxisCPU()
    {
    ArrayHandle<Dscalar2> h_p(cellPositions,access_location::host,access_mode::readwrite);
    ArrayHandle<Dscalar2> h_v(vertexPositions,access_location::host,access_mode::read);
    ArrayHandle<int> h_nn(cellVertexNum,access_location::host,access_mode::read);
    ...
    for (int cell = 0; cell < Ncells; ++cell)
        ...
        baseVertex = h_v.data[h_n.data[n_idx(0,cell)]];
        int neighs = h_nn.data[cell];
	//Dscalar2 Putvalue = vertices[0];
        vector<Dscalar2> vertices(neighs+1,zero);
        //vector<Dscalar2> vertices(neighs+1,vertices[0]);
        for (int vv = 1; vv < neighs; ++vv)
            {
            int vidx = h_n.data[n_idx(vv,cell)];
            Box->minDist(h_v.data[vidx],baseVertex,vertices[vv]);
	...
	Dscalar sxy=0.0;
        ...
        centroid.x = centroid.x / (6.0*Area);
        centroid.y = centroid.y / (6.0*Area);
//	For each segment defined by two consecutive points of the polygon, consider a triangle with two
//      corners at these points and third corner at the origin of the coordinates. Integration by the area of
//      that triangle and summing by the polygon segments yields: 
        sxx = sxx/12 - (Area*centroid.y*centroid.y);
        syy = syy/12 - (Area*centroid.x*centroid.x);
        sxy = sxy/24 - (Area*centroid.x*centroid.y);	
	//printf("inertia %f\t%f\t%f\n",sxx,syy,sxy);
	Eigen::Matrix<double, 2, 2> A;
	//The second moment of area about the origin for any simple polygon on the XY-plane 
	//can be computed in general by summing contributions from each segment of the polygon 
	//after dividing the area into a set of triangles. This formula is related to the shoelace 
	//formula and can be considered a special case of Green's theorem.
	A(0,0) = sxx; A(0,1) = sxy; A(1,0) = sxy; A(1,1) = syy;
	Eigen::EigenSolver<Eigen::Matrix<double, 2,2> > s(A);
	double E1= s.eigenvalues()[0].real();
	double E2= s.eigenvalues()[1].real();
	//std::cout << s.eigenvalues() << std::endl;
	E1 = abs(E1);E2=abs(E2);
	std::vector<double> Eigenvalue;
	Eigenvalue.push_back(E1);
	Eigenvalue.push_back(E2);
	std::sort( Eigenvalue.begin(), Eigenvalue.end() );
	MajMinaxissquare.x= Eigenvalue[0];//minor axis
	MajMinaxissquare.y= Eigenvalue[1];//Major axis
	//printf("sort axis %f\t%f\t%f\n",MajMinaxissquare.x,MajMinaxissquare.y,sqrt(MajMinaxissquare.y/MajMinaxissquare.x));
        Box->putInBoxReal(centroid);
        h_p.data[cell] = MajMinaxissquare;
	Eigenvalue.clear();
        };
    };
/*!
...
void vertexModelBase::testAndPerformT1TransitionsCPU()
    ...
    //cout<<"T1T1Threshold "<<T1Threshold<<endl;
    Dscalar2 edge;
