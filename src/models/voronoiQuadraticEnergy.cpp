...
/*	rik.x = nnextp.x-pi.x;
    ...
    for(int nn = 0; nn < neigh; ++nn)
        ...
        Dscalar2 r1,r2,r3;
        Box->minDist(nl1,pi,r1); //open it
            /******ITAH EDIT*********/
 /*           r1.x = nl1.x-pi.x;
            r1.y = nl1.y-pi.y;
            while(r1.x < -0.5*(x11-x12)) r1.x += (x11-x12);
            while(r1.y < -0.5*(x22-x21)) r1.y += (x22-x21);
            while(r1.x > 0.5*(x11-x12)) r1.x -= (x11-x12);
            while(r1.y > 0.5*((x22-x21))) r1.y -= (x22-x21);*/
          /***********END***************/
        Box->minDist(nn1,pi,r2); //open it
            /******ITAH EDIT*********/
/*            r2.x = nn1.x-pi.x;
            r2.y = nn1.y-pi.y;
            while(r2.x < -0.5*(x11-x12)) r2.x += (x11-x12);
            while(r2.y < -0.5*(x22-x21)) r2.y += (x22-x21);
            while(r2.x > 0.5*(x11-x12)) r2.x -= (x11-x12);
            while(r2.y > 0.5*((x22-x21))) r2.y -= (x22-x21);*/
	/**********END***********/
        Box->minDist(no1,pi,r3); //open it
            /******ITAH EDIT*********/
/*            r3.x = no1.x-pi.x;
            r3.y = no1.y-pi.y;
            while(r3.x < -0.5*(x11-x12)) r3.x += (x11-x12);
            while(r3.y < -0.5*(x22-x21)) r3.y += (x22-x21);
            while(r3.x > 0.5*(x11-x12)) r3.x -= (x11-x12);
            while(r3.y > 0.5*((x22-x21))) r3.y -= (x22-x21);*/
        /**********END***********/
