...
void voronoiModelBase::movePointsCPU(GPUArray<Dscalar2> &displacements,Dscalar scale)
    ...
    Dscalar x11,x12,x21,x22; //ITAH ADDED
    if(scale == 1.)
        {
//	Box->getBoxDims(x11,x12,x21,x22); //ITAH ADDED
	//cout<<"pagla\n"<<endl;
        for (int idx = 0; idx < Ncells; ++idx)
            {
            h_p.data[idx].x += h_d.data[idx].x;
            h_p.data[idx].y += h_d.data[idx].y;
//	    cout<<h_p.data[idx].x<<"\t"<<h_p.data[idx].y<<endl;
            Box->putInBoxReal(h_p.data[idx]); //open it
//	    cout<<h_p.data[idx].x<<"\t\t"<<h_p.data[idx].y<<endl;
/*                while(h_p.data[idx].x < x12)
        	{
        	 h_p.data[idx].x +=(x11-x12);
        	};
    		while(h_p.data[idx].y < x21)
        	{
        	 h_p.data[idx].y +=(x22-x21);
        	};
    		while(h_p.data[idx].x>=x11)
        	{
        	h_p.data[idx].x -= (x11-x12);
        	};
    		while(h_p.data[idx].y>=x22)
        	{
        	h_p.data[idx].y -= (x22-x21);
        	};*/
            };
...
void voronoiModelBase::testTriangulationCPU()
	...
	//RearrangedIDs.clear();
	    ...
	    //cout<<"neighbour number"<<neighbors.size()<<"for cell"<<nn<<endl;
		...
		RearrangedIDs.push_back(nn);
		//cout<<"particle topology update"<<nn<<endl;
                };
            };

	   //cout<<"Size"<<RearrangedIDs.size()<<endl;
        };
    };
...
void voronoiModelBase::computeGeometryCPU()
    ...
    for (int i = 0; i < Ncells; ++i)
        ...
        Box->minDist(nlastp,pi,rij); //open it
	/******ITAH EDIT*********/
        /*Dscalar x11,x12,x21,x22;
        Box->getBoxDims(x11,x12,x21,x22);
        rij.x = nlastp.x-pi.x;
        rij.y = nlastp.y-pi.y;;
        while(rij.x < -0.5*(x11-x12)) rij.x += (x11-x12);
        while(rij.y < -0.5*(x22-x21)) rij.y += (x22-x21);
        while(rij.x > 0.5*(x11-x12)) rij.x -= (x11-x12);
        while(rij.y > 0.5*((x22-x21))) rij.y -= (x22-x21);*/
	/***********END***************/
        for (int nn = 0; nn < neigh;++nn)
            ...
            Box->minDist(nnextp,pi,rik); //open it
	    /******ITAH EDIT*********/
	  /*  rik.x = nnextp.x-pi.x;
       	    rik.y = nnextp.y-pi.y;
            while(rik.x < -0.5*(x11-x12)) rik.x += (x11-x12);
            while(rik.y < -0.5*(x22-x21)) rik.y += (x22-x21);
            while(rik.x > 0.5*(x11-x12)) rik.x -= (x11-x12);
            while(rik.y > 0.5*((x22-x21))) rik.y -= (x22-x21);*/
	  /***********END***************/
