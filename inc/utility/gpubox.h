...
/*********ITAH ADDED*************/
	HOSTDEVICE void putInBoxReal1(Dscalar2 &p1);
	HOSTDEVICE void setSquare1(Dscalar x, Dscalar y, Dscalar x1, Dscalar y1);
/*********END********************/
...
/*******ITAH ADDED**************/
	HOSTDEVICE void putInBox1(Dscalar2 &vp);
/********END************/
    };
...
/***ITAH EDIT FOR PBC******/
void gpubox::setSquare1(Dscalar x, Dscalar y,Dscalar x1, Dscalar y1)
    {
    x11=x;x22=y;
    x12=x1;x21=y1;
    /*xi11 = 1./x11;xi22=1./x22;
...
void gpubox::putInBox1(Dscalar2 &vp)
    {
    while(vp.x < x12) 
	{
	vp.x +=(x11-x12);
...
/****Close for 1x and 1y compression edited ITAH**********/
	else{
	Dscalar x11,x12,x21,x22;
	getBoxDims(x11,x12,x21,x22);
        pans.x = p1.x-p2.x;
        pans.y = p1.y-p2.y;;
        while(pans.x < -halfx11) pans.x += (x11-x12);
        while(pans.y < -halfx22) pans.y += (x22-x21);
        while(pans.x > halfx11) pans.x -= (x11-x12);
        while(pans.y > halfx22) pans.y -= (x22-x21);
        }
